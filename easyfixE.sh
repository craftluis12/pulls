...
#Encryption
    echo "Encrypting /dev/$part3" #Encryption Part

    #mkfs.ext4 /dev/$part3 #Old regular format

    cryptsetup luksFormat /dev/$part3 #Encryption Part
    cryptsetup open /dev/$part3 cryptroot #Encryption Part

    mkfs.ext4 /dev/mapper/cryptroot #New Encrypted format

    echo "sda3 format done!" #Encryption Part
...
#Mounting
    #mount /dev/$part3 /mnt #Old Mount 
    mount /dev/mapper/cryptroot /mnt #New Encrypt Mount
    echo "sda3 mounted!"
...
#Crypt setup
    rootdev=$(lsblk -o NAME,TYPE | grep cryptroot | awk '{print $1}')
    realdev=$(lsblk -no PKNAME /dev/mapper/cryptroot)
    uuid=$(blkid -s UUID -o value /dev/$realdev)
    echo "cryptroot UUID=$uuid none luks" > /etc/crypttab.initramfs
...
#Creating Arch Boot Entry
    ...
    echo "options rd.luks.name=$uuid=cryptroot root=/dev/mapper/cryptroot rw quiet" >> /boot/loader/entries/arch.conf
    exit 0
